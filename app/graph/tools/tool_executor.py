# app/graph/tools/tool_executor.py
...
async def call_express_execution_sandbox(
    language: str, code: str, input_data: str = ""
) -> dict[str, Any]:
    """
    ...
    payload = {"language": language, "code": code, "input": input_data}
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if response.status_code != 200:
                logger.error(
                    "Express sandbox returned status code %d", response.status_code
                )
                return {
                    "success": False,
                    "stdout": "",
        ...
        logger.exception(
            "Failed to establish connection with Express execution service"
        )
        return {
            "success": False,
            "stdout": "",
